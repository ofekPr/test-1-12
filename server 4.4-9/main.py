...
def get_file_data(filename):
    ...
    if '/calculate-next' in filename:
        if "?num=" in filename:
            filename = filename.split("num=")[1]
            return str(int(filename) + 1).encode(), 403
        return "0".encode(), 403
    if '/calculate-area' in filename:
        if "?width=" in filename or "?height=" in filename:
            height = filename.split("height=")[1].split("&")[0]
            print(f"{height=}")
            width = filename.split("width=")[1]
            print(f"{width=}")
            return str(int(width) * int(height) / 2).encode(), 403
        return "0".encode(), 403
